/* Extracted JS from whereToBuy.html */
// Modal functions for cities section
                ...
                // Add hover effects for store items
...
// Namespaced Testimonial slider (no conflicts)
            let sahTestiIndex = 1;
            function sahCurrentSlide(n) { sahShowSlide(sahTestiIndex = n); }
            function sahShowSlide(n) {
                const root = document.getElementById('sah-testimonial-slider')?.parentElement || document;
                const slides = root.querySelectorAll('#sah-testimonial-slider .testimonial-item');
                const dots = root.parentElement.querySelectorAll('.testimonial-dots .dot');
                if (!slides.length) return;
                if (n > slides.length) { sahTestiIndex = 1; }
                if (n < 1) { sahTestiIndex = slides.length; }
                slides.forEach(s => s.classList.remove('active'));
                dots.forEach(d => d.classList.remove('active'));
                slides[sahTestiIndex - 1].classList.add('active');
                dots[sahTestiIndex - 1].classList.add('active');
            }
            // Auto-advance
            setInterval(function () { sahTestiIndex++; sahShowSlide(sahTestiIndex); }, 5000);
            // Init
            sahShowSlide(sahTestiIndex);
